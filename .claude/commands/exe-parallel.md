...
RUN `eza src --tree --git-ignore`
RUN `eza app --tree --git-ignore`
...
Make sure agents don't run npm run dev or any other script that would start the Next.js server - focus on the code changes only.